...
## 1. 概要
このプログラムは，ネットワーク接続をしているPicoの 状態管理や，
計測データ取得などの様々なコマンドを統括するプログラムである．

## 2. 動作フロー
1) main()を呼び出す（動作フローの開始点）
2) pico_configsに，接続デバイス情報を登録する
3) すべてのデバイスを統括するインスタンスを作成する
4) 接続テストを実施する
5) コマンド一覧を表示して選択させる
    ping      - 全Picoにpingを送信
    sensor    - 全Picoからセンサーデータを取得
    monitor   - センサーデータの定期取得を開始/停止
    csv       - CSV保存の開始/停止
    led       - 指定PicoのLED制御
    status    - 全Picoのステータスを取得
    quit      - 終了
6) メニュー選択と実行を繰り返す (quitで完全に停止)

## 3. 動作原理
各メニュー項目に対応する動作は，あらかじめPicoに書かれており，
Pico側でコマンド待受けがなされている．
client側からは，各コマンド名の文字列をPicoへ送信することで，
コマンドを発行し，対応するレスポンスを受け取ることで，
ひとつのコマンドが完了する（monitorのみ，連続動作となる）
